...
public class ProductosServiceImpl implements ProductosService {
    ...
    public Producto productoCodigo(int cod) {
        return productos.stream().
                filter(producto -> producto.getCodProducto() == cod)
                .findFirst()
                .orElse(null);
    }
    ...
    public Producto eliminarProducto(int cod) {
        ...
        if(producto!=null){
            productos.removeIf(producto1 -> producto1.getCodProducto() == cod);
        }
        return producto;
    }

    @Override
    public Producto actualizarProducto(int cod, double precio) {
        Producto producto = productoCodigo(cod);
        if(producto!=null){
